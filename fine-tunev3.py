...
def imgaug_augmentation(img):
    ...
    #EfficientNetB0의 ImageNet 기준 정규화를 적용한 데이터 증강 함수

    img_uint8 = (img * 255).astype(np.uint8)  # 0~1 -> 0~255 변환
    augmented = aug_seq(image=img_uint8)  # 증강 적용
    img_float32 = augmented.astype(np.float32) / 255.0  # 다시 0~1 변환
    return preprocess_input(img_float32)  # EfficientNet 정규화 적용
"""
...
for DEFAULT_OBJ_TYPE in OBJECT_TYPES :
    ...
    for tainableLen in range(STARTLAYER, STARTLAYER + LAYERS_TRAINABLE):
        ...
        for nodecnt in NODE_COUNT_LIST:
            ...
            model = get_FineTuneModel(backbone, len(categories), tainableLen, nodecnt, dropout_rate=0.3, train_from_block = 'block1', basemodel_trainable = True)
            ...
            earlystopping = tf.keras.callbacks.EarlyStopping(monitor=MONITOR, patience=patience, mode=op_mode)
