...
for DEFAULT_OBJ_TYPE in OBJECT_TYPES :
    ...
    for tainableLen in range(STARTLAYER,STARTLAYER + LAYERS_TRAINABLE):
        ...
        for nodecnt in range (2048, 1023, -1024):
            ...
            def get_model_path(model_type, backbone="resnet50"):
                ...
                model_path = os.path.join(main_path, "{}_{}_{}_finetune_{}_node_{}_weights_".format(model_type, backbone,datetime.now().strftime("%Y%m%d-%H%M%S"),tainableLen,nodecnt))
                return model_path
            ...
            class_weights = class_weight.compute_class_weight(
                class_weight='balanced',  # 첫 번째 인자를 keyword로 전달
                classes=np.unique(train_generator.classes),
                y=train_generator.classes  # 세 번째 인자도 keyword로 전달
                )
            ...
            model_save_filename = "{}_{}_{}_finetune-model_{}_node_{}_epoch_{}_val_acc_{:.4f}.h5".format(class_str,backbone,datetime.now().strftime("%Y%m%d-%H%M%S"),tainableLen,nodecnt,len(acc),val_acc[-1])
            model.save(model_save_filename,)
        
        
        
        #기존 폴더 아래 있는 출력 폴더를 지운다.
        model_path = os.path.join(OBJECT_DETECTION_API_PATH, 'm_' + model_dir)
